...
export default function SearchTypeToggle({ leftSideText, rightSideText, onToggle }: SearchTypeToggle) {
    ...
    let selectedOption = useRef<keyMap>(1);

    const handleToggleOption = () => {
        selectedOption.current = selectedOption.current ? 0 : 1;
        onToggle(sideMapping[selectedOption.current])
    }

    useEffect(handleToggleOption, []);
    ...
    return (
        <figure className={`${poppins.className} w-full h-fit self-center flex items-center justify-center text-base capitalize`}>
            <label className="inline-flex items-center cursor-pointer">
                ...
                <input onChange={handleToggleOption} type="checkbox" value={0} className="sr-only peer " />
